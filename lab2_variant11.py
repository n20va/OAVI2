...
def local_mean_std_min(gray: np.ndarray, window_size: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # Локальный минимум: через явное сканирование окна.
    local_min = np.empty_like(gray_f)
    for y in range(h):
        for x in range(w):
            patch = padded[y : y + window_size, x : x + window_size]
            local_min[y, x] = patch.min()
...
def feng_tan_binarization(
    ...
    if secondary_window % 2 == 0:
        secondary_window += 1
    _, secondary_std, _ = local_mean_std_min(gray, secondary_window)

    dynamic_range = np.maximum(secondary_std, 1e-6)
    alpha2 = k1 * np.power(std / dynamic_range, gamma)
    alpha3 = k2 * np.power(std / dynamic_range, gamma)
